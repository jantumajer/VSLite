function [trw,varargout] = VSLite_v2_3_Acevedo(syear,eyear,phi,T1,T2,M1,M2,T,P,varargin)
...
%                assuming location is in the northern hemisphere). !!! Zmeneno na [-4;12]
...
for cyear=1:length(iyear)      % begin cycling over years
    ...
    for nt = 1:12
      Gr(nt, cyear) = gE.*(gT(nt,cyear)*gM(nt,cyear))
    end
end % end year cycle
